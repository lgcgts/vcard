...
- Data contract (see [shared/card.js](shared/card.js)): expects `name`, `title`, `company`, `phone`, `displayPhone`, `whatsapp`, `emailWork`, `emailPersonal`, `address`, `locationText`, `locationUrl`; optional `website: { url, label }`. Missing `website` hides the website card.
